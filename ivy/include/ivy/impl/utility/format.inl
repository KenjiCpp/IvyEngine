namespace ivy {
	namespace utility {
		...
		namespace details {
			...
			b8 _format(const template_type& _template, string_type& _res, u32 _offset, Args&&... _args) noexcept;

			template<typename T, typename... Args>
			b8 _format(const template_type& _template, string_type& _res, u32 _offset, T&& _t, Args&&... _args) noexcept {
				...
				if (utility::find_format_context(_template, _offset, ctx, start, end, err)) {
					...
					return _format(_template, _res, end + 1, std::forward<Args>(_args)...);
				}
				else {
			...
			template<>
			IVY_API b8 _format(const template_type& _template, string_type& _res, u32 _offset) noexcept;
		...
		string_type format(const template_type& _template, Args&&... _args) noexcept {
			...
			if (!details::_format(_template, res, 0, std::forward<Args>(_args)...)) {
